...
function handleSwishRequest(req, res, next) {
  ...
  if (req.sessionID !== headers.swish_sess_id) {
    ...
    req.session.swishdec.next_prv === undefined || req.session.swishdec.created_date === undefined) {
...
module.exports.SwishServer = (req, res, next) => {
  res.sendSwish = function sendSwish(body) {
    if (body === undefined || body === '') {
      body = JSON.stringify({}); //always return a json even empty
    } else {
      if (req.headers !== undefined && typeof req.headers.swish_sess_id === 'string' &&
        req.session !== undefined && req.session.next_req_pub !== undefined) {
        const response = serverHS.encryptResponse(req.headers.swish_sess_id, body, req.session.next_req_pub);
