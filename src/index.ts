...
async function senddSwish(req: Request, res: Response, next: NextFunction, body: Record<string, unknown> = {}): Promise<void> {
  try {
...
export class Swish {
   ...
   */
  async handleSwishRequest(req: Request, _res: Response, next: NextFunction): Promise<void> {
  ...
  async handleSwishSessionDestroy(req: Request, res: Response): Promise<void> {
    if (await this.destroySession(req.swish.sessionId)) {
    ...
    try {
      ...
      if (req.headers['swish-action'] !== undefined) {
        if (req.headers['swish-action'] === 'handshake_init') {
          ...
          await this.handleSwishRequest(req, res, next)
        } else if (req.headers['swish-action'] === 'session_destroy') {
          await this.loadSession(req)
          validateRequestSession(req)
          await this.handleSwishSessionDestroy(req, res)
        } else {
